...
CREATE TABLE `User` (
    ...
    `defaultOrganizationId` INTEGER NULL,
...
CREATE TABLE `Farm` (
    ...
    `areaUnit` VARCHAR(191) NULL,
    `photo` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NULL,

    UNIQUE INDEX `Farm_organizationId_name_key`(`organizationId`, `name`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `FinancialReport` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `farmId` INTEGER NOT NULL,
    `type` ENUM('PROFIT_LOSS', 'CASHFLOW', 'BALANCE_SHEET', 'EXPENSE_ANALYSIS', 'REVENUE_ANALYSIS') NOT NULL,
...
CREATE TABLE `Lot` (
    ...
    `ageGroup` ENUM('young', 'adult', 'breeding', 'fattening', 'mixed') NULL,
...
CREATE TABLE `Birth` (
    ...
    `notes` VARCHAR(191) NULL,
    `userId` INTEGER NULL,
    `animalReproductionId` INTEGER NULL,
    `gestationId` INTEGER NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `Birth_gestationId_key`(`gestationId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Animal` (
    ...
    UNIQUE INDEX `Animal_qrcode_key`(`qrcode`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `expenses` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `farmId` INTEGER NOT NULL,
    `category` ENUM('FEED', 'VETERINARY', 'EQUIPMENT', 'MAINTENANCE', 'LABOR', 'FUEL', 'FERTILIZER', 'SEEDS', 'WATER', 'TRANSPORT', 'INSURANCE', 'TAXES', 'SUPPLIES', 'UTILITIES', 'MARKETING', 'MISC', 'OTHER') NOT NULL,
    `amount` DOUBLE NOT NULL,
    `taxAmount` DOUBLE NULL DEFAULT 0,
    `totalAmount` DOUBLE NOT NULL,
    `date` DATETIME(3) NOT NULL,
    `paymentMethod` ENUM('card', 'mobile_money', 'orange_money', 'paypal', 'cash', 'others') NOT NULL DEFAULT 'cash',
...
CREATE TABLE `Sale` (
    ...
    INDEX `Sale_farmId_date_idx`(`farmId`, `date`),
    INDEX `Sale_farmId_status_idx`(`farmId`, `status`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `inventory` (
    ...
    `farmId` INTEGER NOT NULL,
    `name` VARCHAR(191) NOT NULL,
    `category` ENUM('FEED', 'MEDICINE', 'SUPPLEMENT', 'FERTILIZER', 'SEED', 'EQUIPMENT', 'TOOL', 'CHEMICAL', 'PACKAGING', 'FUEL', 'OTHER') NOT NULL,
    `sku` VARCHAR(191) NULL,
    `quantity` DECIMAL(12, 3) NOT NULL,
    `unit` VARCHAR(191) NOT NULL,
    `minQuantity` DECIMAL(12, 3) NULL,
...
CREATE TABLE `StockMovement` (
    ...
    `type` ENUM('PURCHASE', 'USAGE', 'ADJUSTMENT', 'TRANSFER', 'RETURN', 'WASTE') NOT NULL,
    `quantity` DECIMAL(12, 3) NOT NULL,
    ...
    INDEX `StockMovement_inventoryId_date_idx`(`inventoryId`, `date`),
...
CREATE TABLE `AnimalFeeding` (
    ...
    `feedingPlanId` INTEGER NOT NULL,
    `quantity` DECIMAL(12, 3) NOT NULL,
    `unit` VARCHAR(191) NOT NULL,
    `date` DATETIME(3) NOT NULL,
    `lotId` INTEGER NOT NULL,
    `userId` INTEGER NOT NULL,
    `feedStockId` INTEGER NULL,
...
CREATE TABLE `FeedingPlan` (
    ...
    `herdId` INTEGER NULL,
    `penId` INTEGER NULL,
    `lastDistributedAt` DATETIME(3) NULL,
    `feedStockId` INTEGER NULL,
...
CREATE TABLE `feed_stock` (
    ...
    `farmId` INTEGER NOT NULL,
    `name` VARCHAR(191) NOT NULL,
    `category` ENUM('CONCENTRATE', 'FORAGE', 'SUPPLEMENT', 'MINERAL', 'SILAGE', 'OTHER') NULL,
    `quantity` DECIMAL(12, 3) NOT NULL,
    `unit` VARCHAR(191) NOT NULL,
    `minQuantity` DECIMAL(12, 3) NULL,
    ...
    UNIQUE INDEX `feed_stock_sku_key`(`sku`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Equipment` (
    ...
    `value` DOUBLE NULL,
    `maintenanceFrequency` ENUM('daily', 'weekly', 'monthly', 'quarterly', 'yearly', 'custom') NOT NULL DEFAULT 'daily',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `AnimalMovement` (
    ...
    `fromLotId` INTEGER NULL,
    `toLotId` INTEGER NULL,
    `fromHerdId` INTEGER NULL,
    `toHerdId` INTEGER NULL,
    `date` DATETIME(3) NOT NULL,
    `movedById` INTEGER NULL,
...
CREATE TABLE `Payment` (
    ...
    `method` ENUM('card', 'mobile_money', 'orange_money', 'paypal', 'cash', 'others') NOT NULL,
...
CREATE TABLE `GeneticPerformance` (
    ...
    `birthWeight` DOUBLE NULL,
    `weaningWeight` DOUBLE NULL,
    `prolificityScore` DOUBLE NULL,
    `maternalInstinct` INTEGER NULL,
    `diseaseResistance` INTEGER NULL,
    `inbreedingCoeff` DOUBLE NULL,
    `updatedAt` DATETIME(3) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `purchases` ADD CONSTRAINT `purchases_createdById_fkey` FOREIGN KEY (`createdById`) REFERENCES `User`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `PurchaseItem` ADD CONSTRAINT `PurchaseItem_purchaseId_fkey` FOREIGN KEY (`purchaseId`) REFERENCES `purchases`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `FinancialReport` ADD CONSTRAINT `FinancialReport_farmId_fkey` FOREIGN KEY (`farmId`) REFERENCES `Farm`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `FinancialReport` ADD CONSTRAINT `FinancialReport_generatedById_fkey` FOREIGN KEY (`generatedById`) REFERENCES `User`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `expenses` ADD CONSTRAINT `expenses_farmId_fkey` FOREIGN KEY (`farmId`) REFERENCES `Farm`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `expenses` ADD CONSTRAINT `expenses_supplierId_fkey` FOREIGN KEY (`supplierId`) REFERENCES `Supplier`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `FeedPurchase` ADD CONSTRAINT `FeedPurchase_inventoryId_fkey` FOREIGN KEY (`inventoryId`) REFERENCES `inventory`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `FeedUsage` ADD CONSTRAINT `FeedUsage_feedStockId_fkey` FOREIGN KEY (`feedStockId`) REFERENCES `feed_stock`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `AnimalFeeding` ADD CONSTRAINT `AnimalFeeding_feedingPlanId_fkey` FOREIGN KEY (`feedingPlanId`) REFERENCES `FeedingPlan`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `AnimalFeeding` ADD CONSTRAINT `AnimalFeeding_feedStockId_fkey` FOREIGN KEY (`feedStockId`) REFERENCES `feed_stock`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `FeedingPlan` ADD CONSTRAINT `FeedingPlan_inventoryId_fkey` FOREIGN KEY (`inventoryId`) REFERENCES `inventory`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `FeedingPlan` ADD CONSTRAINT `FeedingPlan_feedStockId_fkey` FOREIGN KEY (`feedStockId`) REFERENCES `feed_stock`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `feed_stock` ADD CONSTRAINT `feed_stock_farmId_fkey` FOREIGN KEY (`farmId`) REFERENCES `Farm`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `feed_stock` ADD CONSTRAINT `feed_stock_supplierId_fkey` FOREIGN KEY (`supplierId`) REFERENCES `Supplier`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Equipment` ADD CONSTRAINT `Equipment_inventoryId_fkey` FOREIGN KEY (`inventoryId`) REFERENCES `inventory`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Invoice` ADD CONSTRAINT `Invoice_farmId_fkey` FOREIGN KEY (`farmId`) REFERENCES `Farm`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Client` ADD CONSTRAINT `Client_farmId_fkey` FOREIGN KEY (`farmId`) REFERENCES `Farm`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
